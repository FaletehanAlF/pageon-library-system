<div class="flex min-h-screen items-center justify-center p-4 py-10">
    <div class="w-full max-w-md">

        <div class="mb-8 text-center">
            <p class="text-3xl font-bold tracking-tight">Daftar Admin</p>
            <p class="mt-2 text-sm text-gray-500">Maksimal 2 admin. Wajib memakai kode invite dari owner.</p>
        </div>

        <div class="rounded-2xl border border-amber-200 bg-white p-8 shadow-sm">

            <?php $flashError = Session::getFlash('error'); ?>
            <?php if ($flashError): ?>
                <div role="alert" class="mb-6 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800"><?= e((string) $flashError) ?></div>
            <?php endif; ?>

            <?php $fieldErrors = Session::getFlash('errors'); ?>
            <?php if (is_array($fieldErrors) && $fieldErrors !== []): ?>
                <div role="alert" class="mb-6 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800">
                    <ul class="list-disc list-inside space-y-1">
                        <?php foreach ($fieldErrors as $msg): ?>
                            <li><?= e((string) $msg) ?></li>
                        <?php endforeach; ?>
                    </ul>
                </div>
            <?php endif; ?>

            <form method="POST" action="<?= e($portalAction) ?>" class="space-y-5" novalidate>
                <?= csrf_field() ?>

                <div>
                    <label for="name" class="mb-1.5 block text-sm font-medium text-gray-700">Nama Lengkap</label>
                    <input type="text" id="name" name="name" value="<?= old('name') ?>" required maxlength="100"
                        class="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm outline-none focus:border-gray-900 focus:bg-white">
                </div>

                <div>
                    <label for="email" class="mb-1.5 block text-sm font-medium text-gray-700">Email</label>
                    <input type="email" id="email" name="email" value="<?= old('email') ?>" required maxlength="100"
                        class="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm outline-none focus:border-gray-900 focus:bg-white">
                </div>

                <div class="grid sm:grid-cols-2 gap-4">
                    <div>
                        <label for="password" class="mb-1.5 block text-sm font-medium text-gray-700">Password (min 8)</label>
                        <input type="password" id="password" name="password" required autocomplete="new-password"
                            class="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm outline-none focus:border-gray-900 focus:bg-white">
                    </div>
                    <div>
                        <label for="password_confirmation" class="mb-1.5 block text-sm font-medium text-gray-700">Konfirmasi</label>
                        <input type="password" id="password_confirmation" name="password_confirmation" required autocomplete="new-password"
                            class="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm outline-none focus:border-gray-900 focus:bg-white">
                    </div>
                </div>

                <div>
                    <label for="invite_code" class="mb-1.5 block text-sm font-medium text-gray-700">Kode Invite Owner</label>
                    <input type="text" id="invite_code" name="invite_code" required autocomplete="off"
                        class="w-full rounded-xl border border-amber-300 bg-amber-50 px-4 py-3 text-sm font-mono tracking-widest outline-none focus:border-gray-900 focus:bg-white"
                        placeholder="XXXX-XXXX-XXXX">
                </div>

                <button type="submit" class="w-full rounded-xl bg-gray-900 px-4 py-3 text-sm font-medium text-white hover:bg-gray-800 transition">
                    Buat Akun Admin
                </button>
            </form>

        </div>

        <p class="mt-6 text-center text-sm text-gray-500">
            Sudah punya akun? <a href="<?= e($portalLogin) ?>" class="font-medium text-gray-900 hover:underline">Masuk portal</a>
        </p>

    </div>
</div>
