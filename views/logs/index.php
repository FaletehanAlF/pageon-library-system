<div class="page-header">
    <div>
        ...
        <p class="page-subtitle">Audit admin — siapa melakukan apa. Total: <span class="font-semibold text-gray-700"><?= (int) ($total ?? 0) ?></span>.</p>
    </div>
</div>
    ...
    <div class="table-scroll">
        <table class="table-base">
            <thead><tr class="border-b bg-gray-50 text-left text-gray-500">
                <th class="px-5 lg:px-6 py-4 font-medium">Waktu</th>
                <th class="px-5 lg:px-6 py-4 font-medium">User</th>
                <th class="px-5 lg:px-6 py-4 font-medium">Aksi</th>
                <th class="px-5 lg:px-6 py-4 font-medium">Detail</th>
                <th class="px-5 lg:px-6 py-4 font-medium">IP</th>
            </tr></thead>
            <tbody>
                <?php if (empty($logs)): ?>
                ...
                <?php else: ?>
                    <?php foreach ($logs as $l): ?>
                        <tr class="border-b border-gray-50 hover:bg-gray-50/50">
                            <td class="px-5 lg:px-6 py-4 text-gray-500 whitespace-nowrap" title="<?= e($l['created_at']) ?>"><?= e(time_ago($l['created_at'])) ?></td>
                            <td class="px-5 lg:px-6 py-4"><?= e($l['user_name'] ?? ('#' . ($l['user_id'] ?? '-'))) ?></td>
                            <td class="px-5 lg:px-6 py-4"><span class="rounded-full bg-gray-100 px-2.5 py-1 font-mono text-xs"><?= e($l['action']) ?></span></td>
                            <td class="px-5 lg:px-6 py-4 text-gray-600 max-w-md truncate" title="<?= e($l['detail'] ?? '') ?>"><?= e($l['detail'] ?? '-') ?></td>
                            <td class="px-5 lg:px-6 py-4 font-mono text-xs text-gray-400"><?= e($l['ip'] ?? '-') ?></td>
                        </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
            </tbody>
